...
// Function to handle closing image dialog
  ...
  const syncDeletedRecords = async () => {
    ...
    console.log(deletedObjectStore)
  
    try {
      const deletedRecordsBeforeSync = await deletedObjectStore.getAll();
      console.log('Deleted records before sync:', deletedRecordsBeforeSync[0]);
      ...
      for (const { data , file, _id } of deletedRecordsBeforeSync) {
        if (data === undefined){
          ...
          for (const { firstName, lastName, email, phoneNumber, dob, gender, height, weight, bmi, disease, timestamp , file, _id } of deletedRecordsBeforeSync) {
